...
export const RoleEditScreen: React.FC = () => {
  ...
  // Populate data when role is loaded
  useEffect(() => {
    if (role) {
      setName(role.name || '');
      setDescription(role.description || '');
      setStatus((role.status as 'ACTIVE' | 'INACTIVE') || 'ACTIVE');
      setSelectedPermissions(role.menuAccess || role.permissions || []);
    }
  }, [role]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(
        err?.response?.data?.message || err?.message || 'Gagal memperbarui data role'
      );
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="py-20">
        <LoadingState message="Memuat formulir edit role..." />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-24">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <span>/</span>
            <Link to={`/settings/roles/${id}`} className="hover:text-[#0D5C53]">
              {role.name}
            </Link>
            <span>/</span>
            <span className="text-slate-800 font-semibold">Edit Role</span>
          </div>
      ...
      {/* Validation / Error Banner (Matching Figma Screen 4) */}
      {formError && (
      ...
      {selectedPermissions.length === 0 && (
        <div className="p-4 rounded-xl bg-rose-50/80 border border-rose-200 text-rose-800 text-xs flex items-center gap-2.5">
          <AlertTriangle className="w-5 h-5 text-rose-600 shrink-0" />
          <span>
            <strong>Permission Required:</strong> Minimal pilih satu izin akses untuk role ini agar staf dapat beroperasi.
          </span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <FormTextarea
                ...
                placeholder="Jelaskan cakupan wewenang role..."
              />
            </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              <div>
                ...
                <span className="text-[11px] text-slate-500">
                  Role nonaktif tidak dapat dipilih saat mendaftarkan staf baru
                </span>
              </div>
        ...
        {/* Card 2: Access Permissions (Matching Figma Screen 4) */}
        <Card
          header={
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Centang modul dan fitur yang diizinkan untuk role {name || role.name}.
                </p>
              </div>
